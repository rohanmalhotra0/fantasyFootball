...
/** Reusable stepper: minus / number input / plus. Also used by the
 *  Settings page for teams + draft slot. */
export function Stepper({ label, value, min, max, onChange, testId }: StepperProps) {
  const clamp = (n: number) => Math.min(max, Math.max(min, n))
  const btn =
    'h-12 w-12 rounded-xl border-2 border-slate-300 bg-white text-xl font-bold text-slate-800 hover:bg-slate-100 disabled:opacity-40'
  return (
    <div className="flex items-center gap-3">
      ...
      <input
        type="number"
        inputMode="numeric"
        className="h-12 w-20 rounded-xl border-2 border-slate-300 text-center text-lg font-bold"
        value={value}
        min={min}
        max={max}
...
export default function RosterEditor({ roster, onChange }: RosterEditorProps) {
  return (
    <div className="space-y-5">
      ...
      <p data-testid="rounds-readout" className="text-xl font-bold">
        <span aria-hidden="true">📋</span> Rounds: {rosterTotal(roster)}
      </p>
      <p className="text-base text-slate-600">One round per roster spot — starters plus bench.</p>
    </div>
  )
}
